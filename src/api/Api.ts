...
export default class Api {
  ...
  private deviceActionCallback: any;
  ...
  constructor(
    ...
    this.deviceActionCallback = deviceActionCallback;
    this.router = Router();
    ...
    this.router.post("/devices/:id/action", (req: Request, res: Response) => {
      const id = req.params.id;
      const action = req.body?.action;
      const entityId = req.body?.entityId;
      logger.info("device " + id + " action "+entityId+"."+action);
      this.deviceActionCallback(id,entityId,action);
      res
        .header("Access-Control-Allow-Origin", "*")
        .status(StatusCodes.OK)
        .json({});
